...
module.exports = {
      ...
      if (isDefaultUser == null || isDefaultUser.length == 0) {
        let insertedUser = await Sys.App.Services.UserServices.insertUserData({
          ...
          password: bcrypt.hashSync(
            Sys.Config.App.defaultUserLogin.password,
  ...
  postLogin: async function (req, res) {
    try {
      ...
      let player = null;
      player = await Sys.App.Services.UserServices.getUserData({
  ...
  forgotPasswordSendMail: async function (req, res) {
    try {
      ...
      if (user == null || user.length == 0) {
        req.flash(
          "error",
          "No Such User Found,Please Enter Valid Registered Email."
        );
      ...
      defaultTransport.sendMail(mailOptions, function (err) {
        if (!err) {
          ...
          req.flash(
            "error",
            "Error sending mail,please try again After some time."
          );
          return res.redirect("/forgot-password");
        }
      });
  ...
  postResetPassword: async function (req, res) {
    try {
      ...
      req.flash(
        "success",
        "Password updated successfully,Now you can Login with your New Pasword."
      );
      return res.redirect("/");
    } catch (e) {
      //console.log("Error",e);
