...
module.exports = {
  ...
  initiateTransaction: async function (req, res) {
    try {
      ...
      if (amount < 100) {
        return res.send({
          status: "Error",
          response: "Minimum Deposit should be 100 or Greater",
        });
      }
      ...
      let paymentLinkData =
        await Sys.App.Services.PaymentServices.getpaymentLink(payment_data);
      //console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
      //console.log("Order ID :",paymentLinkData.order_id);
      //console.log("Playment Data :",paymentLinkData.short_url);
      //console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
      ...
      let transaction =
        await Sys.App.Services.TransactionServices.createTransaction(data);
      let lockCash = 0;
      let requiredRewardPoint = 0;
      // here comes the validation of promo code and calculation of lock/loyality points as per that.
      if (req.body.promoCode) {
        if (!amount || amount < 100) {
          return res.send({
            status: "Error",
            response: "Deposit amount should be 100 or more",
          });
        }
        //console.log("###########################################################");
        //console.log("###########################################################");
        ...
        //console.log("###########################################################");
        // check for any previous deposits
        let previousDepositCount =
          await Sys.App.Services.TransactionServices.getDepositCount({
            playerId: req.body.playerId,
            status: "Success",
          });
        // if no deposit present then implement code of welcome
        ...
        if (
          ...
          req.body.promoCode.toUpperCase() == "WELCOME"
        ) {
          // 100-999 => get locked bonus, 25% of the value of deposit value
          // 1000-1999 => get locked bonus, 50% of the value of deposit value
          // 2000 or more => get locked bonus, 100% of amount but max 3000.
        ...
        // if single entry present then implment code of firsttopup
          ...
          previousDepositCount == 1 &&
          req.body.promoCode.toUpperCase() == "FIRSTRECHARGE"
        ) {
          // get 50% of the amount but max 3000
          //console.log('FirstRechange');
          let percentage = 50;
        ...
        // if 2 or more entry is present then proceed as per the promo code present in the system(if it is a valid code)
          ...
          //console.log("beyond 3rd transaction with promocode", req.body.promoCode);
          let validPromoCode =
            await Sys.App.Services.PromocodeServices.getSinglePromocodeData({
          ...
          if (validPromoCode) {
            ...
            let validPromoCodeUsed = 0;
            validPromoCodeUsed =
              await Sys.App.Services.PromocodeServices.getHistoryCount({
                promocode: req.body.promoCode,
                playerId: req.body.playerId,
              });
            if (validPromoCodeUsed < validPromoCode.individual_usage_limit) {
              ...
              return res.send({
                status: "Error",
                response: "Promo Code Already Used Maximum Times",
              });
            }
          } else {
            return res.send({
              status: "Error",
              response: "No Such Valid promoCode Found",
            });
          }
        }
        //console.log("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$");
      ...
      if (player && player.signupReferralCode) {
        ...
        let refferal_player =
          await Sys.App.Services.PlayerServices.getSinglePlayerData({
            myReferralCode: player.signupReferralCode,
          });
        if (refferal_player) {
          ...
          if (previousDepositCount == 0) {
            let previousInstanceBonusDepositCount =
              await Sys.App.Services.TransactionServices.getInstanceBonusDepositCount(
                { refferalPlayerId: refferal_player._id, status: "Success" }
              );
            let refferal_percentage = 0;
            if (
              previousInstanceBonusDepositCount == 0 ||
              previousInstanceBonusDepositCount == 1
            ) {
              refferal_percentage = 5;
            }
            if (
              previousInstanceBonusDepositCount == 2 ||
              previousInstanceBonusDepositCount == 3
            ) {
              refferal_percentage = 10;
            }
            if (
              previousInstanceBonusDepositCount == 4 ||
              previousInstanceBonusDepositCount == 5 ||
              previousInstanceBonusDepositCount == 6
            ) {
              refferal_percentage = 15;
            }
            if (
              previousInstanceBonusDepositCount == 7 ||
              previousInstanceBonusDepositCount == 8
            ) {
            ...
            let refferal_amount = parseFloat(
              (amount * refferal_percentage) / 100
            );
            ...
            await Sys.App.Services.TransactionServices.createInstanceBonusDepositManage(
              refferal_deposit_data
            );
          }
        }
      }
      ...
      let cashManager =
        await Sys.App.Services.TransactionServices.getPlayerCashData({
          playerId: req.body.playerId,
        });
      //console.log("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$");
      //console.log("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$");
      ...
      if (cashManager && cashManager.requiredRewardPoint) {
        rr =
          parseFloat(cashManager.requiredRewardPoint) +
          parseFloat(requiredRewardPoint);
      } else {
        rr = parseFloat(requiredRewardPoint);
      }
      let depositData = {
      ...
      let cashDepositManage =
        await Sys.App.Services.TransactionServices.createCashDepositManage(
          depositData
        );
      return res.send({
        status: "Success",
  ...
  paymentTransaction: async function (req, res) {
    try {
      ...
      if (amount > 25000) {
        return res.send({
          status: "Error",
          response: "Maximum Deposit per day should be 25000 or less.",
        });
      }
      let player = await Sys.App.Services.PlayerServices.getSinglePlayerData({
        _id: req.body.playerId,
      ...
      var TransactionLimit_per_day_cond = {
        $and: [
      ...
      console.log(
        "TransactionLimit_per_day_cond : ",
        TransactionLimit_per_day_cond
      );
      let TransactionLimitPerDayData =
        await Sys.App.Services.TransactionServices.getByData(
          TransactionLimit_per_day_cond
        );
      ...
      var TransactionLimit_per_month_cond = {
        $and: [
          { playerId: req.body.playerId, status: "paid" },
          {
            updatedAt: {
      ...
      console.log(
        "TransactionLimit_per_month_cond : ",
        TransactionLimit_per_month_cond
      );
      let TransactionLimitPerMonthData =
        await Sys.App.Services.TransactionServices.getByData(
          TransactionLimit_per_month_cond
        );
      if (TransactionLimitPerMonthData.length > 0) {
        ...
        if (transaction_total > 250000) {
          return res.send({
            status: "Error",
            response: "Maximum Deposit per month should be 250000 or less.",
          });
        }
        transaction_total = transaction_total + amount;
        if (transaction_total > 250000) {
      ...
      if (req.body.payment_type && req.body.payment_type == "paytm") {
        ...
        paymentLinkData =
          await Sys.App.Services.PaymentServices.getpaytmpaymentLink(
            payment_data
          );
        ...
        paymentLinkData = await Sys.App.Services.PaymentServices.getpaymentLink(
          payment_data
        );
        order_data = paymentLinkData.order_id;
        //console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
  ...
  successTransaction: async function (req, res) {
    ...
    let transaction =
      await Sys.App.Services.TransactionServices.updateTransaction(
        { orderId: req.body.txnid },
        data
      );
    let transactionDone =
      await Sys.App.Services.TransactionServices.getSingleTransaction({
        orderId: req.body.txnid,
      });
    ...
    await Sys.App.Services.TransactionServices.updateManageDeposit(
      { transactionId: req.body.txnid },
      { status: "Success" }
    );
    let depositData =
      await Sys.App.Services.TransactionServices.getSingleDeposit({
        transactionId: req.body.txnid,
      });
    let cashManager =
      await Sys.App.Services.TransactionServices.getPlayerCashData({
        playerId: depositData.playerId,
      });

    let updatedCash =
      parseFloat(cashManager.totalCash) + parseFloat(depositData.depositCash);
    ...
    let cashManageData = {
      ...
      depositedCash:
        parseFloat(cashManager.depositedCash) +
        parseFloat(depositData.depositCash),
    ...
    // update Cash Manager
    await Sys.App.Services.TransactionServices.updateCashManager(
      { playerId: depositData.playerId },
      cashManageData
    );
  ...
  getTransactionHistory: async function (req, res) {
    try {
      ...
      if (req.query.is_date_search == "yes" && search == "") {
        query = {
          createdAt: { $gte: req.query.start_date, $lt: req.query.end_date },
        };
      }
      if (req.query.is_date_search == "yes" && search != "") {
        query = {
      ...
      //console.log(query);
      let transactionC = await Sys.App.Services.TransactionServices.getByData(
        query
      );
      let transactionCount = transactionC.length;
      let data =
        await Sys.App.Services.TransactionServices.getTransactionDatatable(
          query,
          length,
          start
        );
  ...
  checkPaymentInfo: async function (req, res) {
    ...
    try {
      ...
      let updatePaymentData =
        await Sys.App.Services.PaymentServices.updatePaymentStatus(
          pendingTransactions
        );
      ...
      for (var i = 0; i < pendingTransactions.length; i++) {
        ...
        if (
          moment(new Date(pendingTransactions[i].createdAt)).format("DD") !=
            moment(new Date()).format("DD") &&
          moment(new Date(pendingTransactions[i].createdAt)).format("DD") !=
            parseInt(parseInt(moment(new Date()).format("DD")) - 1)
        ) {
      ...
      // //console.log("Pushed1");
      let cancelPaymentStatus =
        await Sys.App.Services.PaymentServices.cancelPaymentStatus(
          expiredTransactions
        );
      // //console.log("Pushed2");
      // res.send("Done")
