...
Sys.Log = winston.createLogger({
  ...
  transports: [
    ...
    new winston.transports.DailyRotateFile({
      filename: path.join(
        Sys.Config.App.logger.logFolder,
        "/" + Sys.Config.App.logger.logFilePrefix + "-%DATE%.log"
      ),
...
fs.readdirSync(path.join(__dirname, "../", "./Game"))
  ...
  .forEach(function (dir) {
    ...
    fs.readdirSync(path.join(__dirname, "../", "./Game", dir))
      .filter(function (file) {
        return file.indexOf(".") !== 0;
      })
      .forEach(function (subDir) {
        ...
        if (fs.existsSync(path.join(__dirname, "../", "./Game", insidePath))) {
          if (
            fs
              .lstatSync(path.join(__dirname, "../", "./Game", insidePath))
              .isFile()
          ) {
            //Sys.Log.info('Loading... File :'+subDir);
            Sys.Game[dir][subDir.split(".")[0]] = require(path.join(
              __dirname,
              "../",
              "./Game",
              dir,
            ...
            fs.readdirSync(path.join(__dirname, "../", "./Game", insidePath))
              .filter(function (file) {
                return file.indexOf(".") !== 0;
              })
              .forEach(function (subInnerDir) {
                ...
                if (
                  fs
                    .lstatSync(
                      path.join(__dirname, "../", "./Game", insidePath)
                    )
                    .isFile()
                ) {
                  ...
                  fs.readdirSync(
                    ...
                    .forEach(function (subInnerLastDir) {
                      ...
                      if (
                        ...
                        Sys.Game[dir][subDir][subInnerDir][
                          subInnerLastDir.split(".")[0]
                        ] = require(path.join(
                          __dirname,
                          "../",
                          "./Game",
                          dir + "/" + subDir + "/" + subInnerDir,
                          subInnerLastDir
                        )); // Add File in Sub Folder Object
                      } else {
...
// Mongodb Connection
var dbURI =
  "mongodb+srv://yuvraj:yuvraj@cluster0.ypdgfyb.mongodb.net/?retryWrites=true&w=majority";
...
mongoose.connection.on("connected", async function () {
  ...
  Sys.Io.on("connection", async function (socket) {
    ...
    Object.keys(Sys.Game.Common.Sockets).forEach(function (key) {
      // Register Socket File in Socket Variable
  ...
  Sys.Io.of(Sys.Config.Namespace.PracticePoints).on(
    "connection",
    function (socket) {
      console.log("User Connected To Practice Points New Namespace");
      Object.keys(Sys.Game.Practice.Points.Sockets).forEach(function (key) {
        // Register Socket File in Socket Variable
  ...
  Sys.Io.of(Sys.Config.Namespace.PracticePool).on(
    "connection",
    function (socket) {
      console.log("User Connected To Practice Pool New Namespace");
      Object.keys(Sys.Game.Practice.Pool.Sockets).forEach(function (key) {
        // Register Socket File in Socket Variable
  ...
  Sys.Io.of(Sys.Config.Namespace.PracticeDeals).on(
    "connection",
    function (socket) {
      console.log("User Connected To Practice Deals New Namespace");
      Object.keys(Sys.Game.Practice.Deals.Sockets).forEach(function (key) {
        // Register Socket File in Socket Variable
  ...
  Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyOne).on(
    "connection",
    function (socket) {
      console.log("User Connected To Practice Points 21 New Namespace");
      Object.keys(Sys.Game.Practice.PointsTwentyOne.Sockets).forEach(function (
        key
      ) {
        // Register Socket File in Socket Variable
  ...
  Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).on(
    "connection",
    function (socket) {
      console.log("User Connected To Practice Points 24 New Namespace");
      Object.keys(Sys.Game.Practice.PointsTwentyFour.Sockets).forEach(function (
        key
      ) {
        // Register Socket File in Socket Variable
  ...
  // Cash Game Points
  Sys.Io.of(Sys.Config.Namespace.CashPoints).on(
    "connection",
    function (socket) {
      console.log("User Connected To Cash Points New Namespace");
      Object.keys(Sys.Game.CashGame.Points.Sockets).forEach(function (key) {
        // Register Socket File in Socket Variable
  ...
  Sys.Io.of(Sys.Config.Namespace.CashPool).on("connection", function (socket) {
    ...
    Object.keys(Sys.Game.CashGame.Pool.Sockets).forEach(function (key) {
      // Register Socket File in Socket Variable
  ...
  Sys.Io.of(Sys.Config.Namespace.CashDeals).on("connection", function (socket) {
    ...
    Object.keys(Sys.Game.CashGame.Deals.Sockets).forEach(function (key) {
      // Register Socket File in Socket Variable
      Sys.Game.CashGame.Deals.Sockets[key](socket);
    });
  });

  Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyOne).on(
    "connection",
    function (socket) {
      console.log("User Connected To Cash Points 21 New Namespace");
      Object.keys(Sys.Game.CashGame.CashTwentyOne.Sockets).forEach(function (
        key
      ) {
        // Register Socket File in Socket Variable
  ...
  Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).on(
    "connection",
    function (socket) {
      console.log("User Connected To Cash Points 24 New Namespace");
      Object.keys(Sys.Game.CashGame.CashTwentyFour.Sockets).forEach(function (
        key
      ) {
        // Register Socket File in Socket Variable
